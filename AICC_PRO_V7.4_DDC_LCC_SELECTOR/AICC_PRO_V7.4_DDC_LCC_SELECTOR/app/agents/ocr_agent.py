...
def configure():
    ...
    for candidate in candidates:
        if candidate and Path(candidate).exists():
            ...
            return
    raise FileNotFoundError("لم يتم العثور على Tesseract OCR")
...
def extract_text(path):
    """Fast adaptive OCR for Render/free CPU.

    One dense-page pass is the normal path. A sparse-text fallback runs only
    when the first pass returns little useful evidence. This prevents 5-10
    image jobs from stalling for minutes while preserving Arabic/English OCR.
    """
    configure()
    attempts = []
    ...
    try:
        result = pytesseract.image_to_string(
            variants[1], lang=languages, config="--oem 3 --psm 6", timeout=10
        )
        result = clean(result)
        if result:
    ...
    primary_chars = (
        len(re.findall(r"[A-Za-z\u0600-\u06FF0-9٠-٩]", attempts[0]))
        if attempts else 0
    )
    if primary_chars < 80:
        try:
            result = pytesseract.image_to_string(
                variants[2], lang=languages, config="--oem 3 --psm 11", timeout=8
            )
            result = clean(result)
            if result:
                attempts.append(result)
    ...
    def strength(value):
        ...
        labels = len(re.findall(
            r"ISBN|ردمك|تأليف|المؤلف|الناشر|الطبعة|حقوق|العنوان", value, re.I
        ))
    ...
    for attempt in attempts:
        for line in attempt.splitlines():
            ...
            if len(key) >= 4 and key not in seen:
                ...
                if digit_count >= 4 or re.search(
                    r"ISBN|ردمك|تأليف|المؤلف|الناشر|الطبعة|حقوق", line, re.I
                ):
                    seen.add(key)
    ...
    if attempts:
        ...
        for candidate in attempts:
            ...
            if base and words:
                overlaps.append(len(base & words) / len(base | words))
